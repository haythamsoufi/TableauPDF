...
class PDFExportApp(QWidget):
    ...
    def __init__(self):
        ...
        self.logic_section_visible = True # Keep logic section visible by default
    ...
    def toggle_mode_selection(self):
        """Shows/hides UI elements based on the selected export mode, ensuring parameters are always visible."""
        ...
        # Always keep the logic container widget visible, as parameters are always allowed
        if hasattr(self, 'logic_container_widget'):
            self.logic_container_widget.setVisible(True) # Ensure container is always visible
        else:
        ...
        # Toggle visibility of "Add Filter" and "Add Condition" buttons
        if hasattr(self, 'add_filter_btn'):
            self.add_filter_btn.setVisible(is_automate_mode)
        if hasattr(self, 'add_condition_btn'):
            self.add_condition_btn.setVisible(is_automate_mode)
        # The "Add Parameter" button should always be visible, no change needed here.
        if hasattr(self, 'add_parameter_btn'):
            self.add_parameter_btn.setVisible(True) # Ensure it's always visible

        # Toggle visibility of existing filter and condition lines
        for filt_info in self.filters:
            if 'hbox' in filt_info and filt_info['hbox']:
                filt_info['hbox'].setVisible(is_automate_mode)
        for cond_info in self.conditions:
            if 'hbox' in cond_info and cond_info['hbox']:
                cond_info['hbox'].setVisible(is_automate_mode)
        # Parameter lines should always be visible, no change needed here.
        for param_info in self.parameters:
            if 'hbox' in param_info and param_info['hbox']:
                param_info['hbox'].setVisible(True) # Ensure it's always visible
        ...
        if not is_automate_mode:
            ...
            # Reset only filters and conditions when switching away from automate mode
            if hasattr(self, 'reset_filters'):
                self.reset_filters()
            if hasattr(self, 'reset_conditions'):
                self.reset_conditions()
            # Do NOT reset parameters here.

        # Adjust window height after visibility changes
    ...
    def on_sheet_selection_changed(self):
        ...
        self.organize_by2_dropdown.clear()
        self.organize_by2_dropdown.addItem("None")
        self.reset_filters() # Reset filters
        self.reset_conditions() # Reset conditions
        # Do NOT reset parameters here, as they are independent of Excel sheet
    ...
    def setup_combined_logic_panel(self, main_content_layout):
        ...
        self.add_filter_btn = QPushButton("➕ Filter") # Store reference
        self.add_filter_btn.setToolTip("Add Excel list filter")
        self.add_filter_btn.clicked.connect(self.add_filter_line)

        self.add_condition_btn = QPushButton("➕ Condition") # Store reference
        self.add_condition_btn.setToolTip("Exclude views based on Excel data")
        self.add_condition_btn.clicked.connect(self.add_condition_line)

        self.add_parameter_btn = QPushButton("➕ Parameter") # Store reference
        self.add_parameter_btn.setToolTip("Override Tableau Parameters")
        self.add_parameter_btn.clicked.connect(self.add_parameter_line)
        ...
        buttons_layout.addWidget(self.add_filter_btn)
        buttons_layout.addWidget(self.add_condition_btn)
        buttons_layout.addWidget(self.add_parameter_btn)
        ...
        # This will be handled by toggle_mode_selection, which is called in initUI
    ...
    def _hide_widgets_in_layout(self, layout_to_hide):
        """Helper function to recursively hide all widgets within a layout."""
        # Ensure the following block is indented correctly under the def statement
        if layout_to_hide is None:
            return
        for i in range(layout_to_hide.count()):
            item = layout_to_hide.itemAt(i)
            if item is not None:
                widget = item.widget()
                if widget is not None:
                    ...
                    sub_layout = item.layout()
                    if sub_layout is not None:
                        self._hide_widgets_in_layout(sub_layout) # Recursive call
    ...
    def setup_control_buttons(self, layout):
        ...
        self.load_btn.setToolTip("Load saved settings or select recent")
        ...
        load_action.triggered.connect(self.load_configuration_file)
        ...
        # *** CORRECTED CONNECTION FOR SAVE BUTTON ***
        self.save_btn.clicked.connect(self.save_configuration_file) # Was self.OnSave
    ...
    def save_configuration_file(self):
        ...
        if file_name: # If the user selected a file (didn't cancel)
            # Ensure the file has a .ini extension
            if not file_name.lower().endswith('.ini'):
                file_name += '.ini'
            self.OnSave(file_name) # Call OnSave with the valid file_name
    ...
    def OnSave(self, file_path):
        ...
        # This method now correctly receives a file_path string or is not called if saving is cancelled.
        if not file_path or not isinstance(file_path, str): # Added safety check
            self.logger.error(f"OnSave called with invalid file_path: {file_path}")
            QMessageBox.critical(self, "Save Error", f"Invalid file path provided for saving: {file_path}")
            return
        ...
        config['General'] = {
            ...
            'trim_pdf_enabled': str(self.trim_pdf_checkbox.isChecked()),
            'merge_pdfs_enabled': str(self.merge_pdfs_checkbox.isChecked()),
        ...
        for idx, filt in enumerate(self.filters):
            ...
            field = "N/A"
            checked_values = []
            apply_param_state = False
            try:
                if filt.get('field_combo'): field = filt['field_combo'].currentText()
                if filt.get('values_combo'): checked_values = filt['values_combo'].getCheckedItemsText()
                if filt.get('apply_as_param_checkbox'): apply_param_state = filt['apply_as_param_checkbox'].isChecked()
            except RuntimeError:
                self.logger.error(f"RuntimeError accessing widgets for Filter {idx} during save. Skipping.")
                continue
            config[section] = {
                'field': field, 'values': ','.join(checked_values), 'apply_as_param': str(apply_param_state)
            }

        config['Conditions'] = {'count': str(len(self.conditions))}
        for idx, cond in enumerate(self.conditions):
            section = f'Condition_{idx}'
            column, type_val, value, excluded = "N/A", "Equals", "", ""
            try:
                ...
                continue
            config[section] = {
                'column': column, 'type': type_val, 'value': value, 'excluded_views': excluded
            }
        ...
        for idx, param in enumerate(self.parameters):
            ...
            try:
                ...
                continue
            config[section] = { 'name': name, 'value': value }

        try:
            # The file_path is now guaranteed to be a string from QFileDialog or load_configuration
            with open(file_path, 'w', encoding='utf-8') as configfile: # Added encoding
    ...
    def OnStart(self):
        ...
        if hasattr(self, 'logic_content_widget') and self.logic_content_widget.isVisible():
            self.logger.info("Auto-contracting filter and condition sections for export.")
            if hasattr(self, 'add_filter_btn'): self.add_filter_btn.setVisible(False)
            if hasattr(self, 'add_condition_btn'): self.add_condition_btn.setVisible(False)
            
            # *** CORRECTED VISIBILITY FOR LAYOUTS ***
            for filt_info in self.filters:
                if 'hbox' in filt_info and filt_info['hbox'] is not None:
                    self._hide_widgets_in_layout(filt_info['hbox']) # Use helper
            
            for cond_info in self.conditions:
                if 'hbox' in cond_info and cond_info['hbox'] is not None:
                    self._hide_widgets_in_layout(cond_info['hbox']) # Use helper
            
            self._adjust_window_height() # Adjust height after visibility changes
    ...
    def set_ui_enabled(self, enabled):
         ...
         # The logic container is always enabled, but its *contents* are toggled by mode.
         # No need to disable the whole logic_container_widget here.
         ...
         # Also disable the add filter/condition/parameter buttons
         if hasattr(self, 'add_filter_btn'): self.add_filter_btn.setEnabled(enabled)
         if hasattr(self, 'add_condition_btn'): self.add_condition_btn.setEnabled(enabled)
         if hasattr(self, 'add_parameter_btn'): self.add_parameter_btn.setEnabled(enabled)
...
# REPLACE this function
    ...
    def export_selected_views_once(self, server, all_views_in_workbook, output_folder, export_format, merge_pdfs_enabled, pdfs_for_single_merge):
        ...
        try:
            ...
            # Parameters are applied here for "Export All Views Once" mode
            # This logic is copied from process_views_for_item, but without item_row dependency
            applied_param_filter_fields = set() # Not strictly needed here as filters are hidden, but for consistency
            self.logger.debug("--- Checking Parameters for 'Export All Views Once' Mode ---")
            for i, param in enumerate(self.parameters):
                try:
                    param_name = param['param_name_txt'].text().strip()
                    param_value_source = param['param_value_txt'].text().strip()
                    ...
                    # In "Export All Views Once" mode, there's no Excel row to get dynamic values from.
                    # So, param_value_source is always the final value.
                    final_param_value = param_value_source

                    self.logger.debug(f"Explicit Param #{i+1}: Attempting to apply vf('{param_name}', '{final_param_value}')")
                    export_options.vf(param_name, final_param_value)
                    self.log_message_signal.emit(f"ℹ Applied Explicit Parameter: {param_name} = {final_param_value}")
                    self.logger.info(f"Applied Explicit Parameter #{i+1}: Name='{param_name}', Value='{final_param_value}'.")

                except RuntimeError:
                    self.logger.error(f"RuntimeError accessing widgets for Explicit Parameter #{i+1}. Skipping parameter.")
                    continue
                except Exception as e:
                    self.logger.error(f"Unexpected error processing Explicit Parameter #{i+1} for 'Export All Views Once': {e}", exc_info=True)
                    continue
            self.logger.debug("--- Finished Checking Explicit Parameters for 'Export All Views Once' Mode ---")
