...
from PyQt5.QtWidgets import (
    ...
    QGroupBox, QDialog, QListWidget, QListWidgetItem, QAbstractItemView, QInputDialog, QFrame
)
...
class PDFExportApp(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle(f'PDF Export Tool {CURRENT_VERSION}')
        ...
        self.check_for_updates()  # Check for updates after initializing the UI

    def initUI(self):
        main_layout = QVBoxLayout()

        # Add Title and GitHub link with a separator line
        title_github_layout = QVBoxLayout()

        title_label = QLabel(f"PDF Export Tool {CURRENT_VERSION}")
        title_label.setFont(QFont('Arial', 20, QFont.Bold))
        title_label.setAlignment(Qt.AlignCenter)

        github_link = QLabel('<a href="https://github.com/haythamsoufi/TableauPDF">TableauPDF on GitHub</a>')
        ...
        title_github_layout.addWidget(separator)

        main_layout.addLayout(title_github_layout)

        # Server Configuration and File Locations side by side
        top_layout = QHBoxLayout()
        self.setup_server_configuration_panel(top_layout)
        self.setup_file_locations_panel(top_layout)
    ...
    def add_condition_line(self, condition=None):
        ...
        if condition:
            ...
            exclude_views = condition.get('excluded_views', [])
    ...
    def select_views(self, hbox, initial_selection):
        ...
        # Fetch the current excluded views from the temporary list
        current_excluded_views = next(
            (cond['excluded_views'] for cond in self.conditions if cond['hbox'] == hbox), []
        )

        for view in self.tableau_views:
            item = QListWidgetItem(view)
            item.setFlags(item.flags() | Qt.ItemIsUserCheckable)
            item.setCheckState(Qt.Checked if view not in current_excluded_views else Qt.Unchecked)
            list_widget.addItem(item)

        list_widget.setSelectionMode(QAbstractItemView.MultiSelection)

        ok_btn = QPushButton("Ok")
        ok_btn.clicked.connect(lambda: self.apply_view_selection(dialog, list_widget, hbox))
    ...
    def apply_view_selection(self, dialog, list_widget, hbox):
        ...
        for index in range(list_widget.count()):
            ...
            if item.checkState() == Qt.Unchecked:
                excluded_views.append(item.text())
        for cond in self.conditions:
            if cond['hbox'] == hbox:
                cond['excluded_views'] = excluded_views
        dialog.accept()
    ...
    def setup_control_buttons(self, layout):
        ...
        load_btn.clicked.connect(self.load_configuration_file)
        save_btn.clicked.connect(self.save_configuration_file)

        layout.addLayout(button_layout)

    def load_configuration_file(self):
        file_name, _ = QFileDialog.getOpenFileName(self, "Load Configuration File", "", "INI Files (*.ini)")
        if file_name:
            self.config_file = file_name
            self.load_configuration()

    def save_configuration_file(self):
        file_name, _ = QFileDialog.getSaveFileName(self, "Save Configuration File", "", "INI Files (*.ini)")
        if file_name:
            self.config_file = file_name
            self.OnSave()
            ...
            for idx in range(conditions_count):
                ...
                if self.config.has_section(section):
                    condition = {
                        ...
                        'excluded_views': self.config.get(section, 'excluded_views', fallback='').split(',')
                    }
    ...
    def run_task(self):
        try:
            ...
            # Refresh the workbook to ensure the latest data is used
            all_workbooks, _ = server.workbooks.get()
            workbook_id = next((workbook.id for workbook in all_workbooks if workbook.name == workbook_name), None)
            if workbook_id is None:
                raise Exception(f"Workbook {workbook_name} not found")
            ...
            for index, row in df.iterrows():
                ...
                for condition in self.conditions:
                    ...
                    if condition['type_choice'].currentText() == 'Equals' and str(row[column]) == value:
                        excluded_views.update(condition['excluded_views'])
    ...
    def OnSave(self):
        ...
        for idx, cond in enumerate(self.conditions):
            ...
            self.config[section] = {
                ...
                'excluded_views': ','.join(cond['excluded_views'])
            }
    ...
    def check_for_updates(self):
        repo_url = "https://api.github.com/repos/haythamsoufi/TableauPDF/releases/latest"
        try:
