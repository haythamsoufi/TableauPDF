...
class PDFExportApp(QWidget):
    ...
    def setup_export_settings_panel(self, main_layout_for_section):
        ...
        # Row 4: Format, Numbering, Buttons
        ...
        # --- Arrange Widgets in Grid (MODIFIED LAYOUT) ---
        ...
        # Row 4 (Format, Numbering, Trim, Reset, Select Views) - **REVISED LAYOUT**
        export_settings_layout.addWidget(self.format_label, 4, 0) # Format Label

        # Format Radio Buttons Layout
        format_layout = QHBoxLayout()
        format_layout.addWidget(self.pdf_radio)
        format_layout.addWidget(self.png_radio)
        format_layout.addStretch()
        export_settings_layout.addLayout(format_layout, 4, 1) # Format radios in column 1

        # Place Checkboxes in separate columns
        export_settings_layout.addWidget(self.numbering_checkbox, 4, 2) # Numbering in column 2

        # --- Define and Place Trim Checkbox ---
        self.trim_pdf_checkbox = QCheckBox("Trim Output") # <--- Changed text
        self.trim_pdf_checkbox.setToolTip(
            ...
            "installed and configured in the script."
        )
        initial_trim_state = getattr(self, 'trim_pdf_enabled', False)
        self.trim_pdf_checkbox.setChecked(initial_trim_state)
        self.trim_pdf_checkbox.setEnabled(self.pdf_radio.isChecked())
        export_settings_layout.addWidget(self.trim_pdf_checkbox, 4, 3) # Trim PDF in column 3
        # --------------------------------------

        # --- Buttons Layout ---
        button_layout_row4 = QHBoxLayout()
        button_layout_row4.setSpacing(10)
        button_layout_row4.addStretch(1) # Push buttons right
        button_layout_row4.addWidget(select_views_btn)
        button_layout_row4.addWidget(reset_settings_btn)
        # Adjust span for buttons (start from column 4 now)
        export_settings_layout.addLayout(button_layout_row4, 4, 4, 1, 4) # Span from col 4 to 7
    ...
    def load_columns_for_filters(self, column_names, specific_filter=None):
        ...
        for filt_info in filters_to_update:
             ...
             if isinstance(filt_info, dict) and 'field_combo' in filt_info and 'values_combo' in filt_info:
                ...
                values_list = filt_info['values_list']
                filter_edit = filt_info['filter_edit']
                ...
                if current_field_selection in column_names:
                    ...
                    self.populate_values_list(field_combo, filter_edit, values_list)
                else:
                    field_combo.setCurrentIndex(-1)
                    field_combo.setPlaceholderText("Select Field")
                    values_list.clear()
                    values_list.setEnabled(False)
                    filter_edit.clear()
    ...
    def setup_control_buttons(self, layout):
        ...
        self.save_btn.clicked.connect(self.save_configuration_file)
    ...
    def save_configuration_file(self):
        ...
        start_dir = os.path.dirname(RECENT_FILES_PATH) if os.path.exists(RECENT_FILES_PATH) else self.current_dir()
        ...
        if file_name:
            if not file_name.lower().endswith('.ini'): file_name += '.ini'
            self.OnSave(file_name)
            self.add_recent_file(file_name) # Add to recent list after successful save
        else: self.logger.info("Configuration file saving cancelled.")
    ...
    def load_tableau_views(self):
        ...
        try:
            ...
            server.add_http_options({'timeout': 60}); server.auth.sign_in_with_personal_access_token(auth)
            self.log_message_signal.emit("Signed in successfully."); self.logger.info("Tableau sign-in successful.")
    ...
    def run_task(self):
        ...
        try:
            ...
            self.logger.info(f"Export settings: Mode={current_mode}, Format={export_format}, Workbook='{workbook_name_to_find}', Output='{output_folder}'")
            ...
            if current_mode == "Automate for a list":
                ...
                if total_items == 0:
                    ...
                    for index, item_row in filtered_df.iterrows():
                        ...
                        org1_val = "".join(c for c in org1_val if c.isalnum() or c in (' ', '_', '-')).strip()
                        org2_val = "".join(c for c in org2_val if c.isalnum() or c in (' ', '_', '-')).strip()

                        item_folder = output_folder
                        if org1_val: item_folder = os.path.join(item_folder, org1_val)
                        if org2_val: item_folder = os.path.join(item_folder, org2_val)
                        ...
                        self.process_views_for_item(server, all_views_in_workbook, item_folder, tableau_filter_field, tableau_filter_value, item_row, export_format)
                ...
                self.export_selected_views_once(server, all_views_in_workbook, item_folder, export_format)
...
# REPLACE this function
    def process_views_for_item(self, server, all_views_in_workbook, item_folder_base, tableau_filter_field, tableau_filter_value, item_row, export_format):
        ...
        try:
            ...
            for view in included_views_for_item:
                ...
                export_file_path = os.path.join(item_folder_final, file_name_final)
    ...
    def export_selected_views_once(self, server, all_views_in_workbook, output_folder, export_format):
        ...
        try:
            ...
            self.logger.info(f"Starting 'Export All Views Once' mode. Format={export_format}, Output='{output_folder}'")
            ...
            for view in selected_views_to_export:
                ...
                file_name_final = f"{prefix}{base_name_clean}.{export_format.lower()}"
                export_file_path = os.path.join(output_folder, file_name_final)

                export_log_msg = f"Exporting view '{view.name}' to '{file_name_final}'..."
                # print(f"  {export_log_msg}") # Less verbose print
                self.logger.debug(export_log_msg)

                # Set export options (no filters/parameters needed here)
                if export_format == "PDF":
                    export_options = PDFRequestOptions(page_type=PDFRequestOptions.PageType.Unspecified, maxage=0)
                else:
                    export_options = ImageRequestOptions(imageresolution=ImageRequestOptions.Resolution.High, maxage=0)
